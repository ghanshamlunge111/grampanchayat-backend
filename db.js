...
function initializeDatabase() {
  db.serialize(() => {
    ...
    db.get('SELECT COUNT(*) as count FROM grampanchayats', [], (err, row) => {
      ...
      if (row.count === 0) {
        db.run("INSERT INTO grampanchayats (gpId, name) VALUES ('gp-parwa', 'पारवा')");
        db.run("INSERT INTO grampanchayats (gpId, name) VALUES ('gp-borwha', 'बोरव्हा')");
        db.run("INSERT INTO grampanchayats (gpId, name) VALUES ('gp-lakhmapur', 'लखमापूर')");
    ...
    db.get('SELECT COUNT(*) as count FROM users', [], async (err, row) => {
      ...
      if (row.count === 0) {
        ...
        // Parwa accounts
        db.run('INSERT INTO users (username, password, role, gpId) VALUES (?, ?, ?, ?)', ['admin-parwa', passwordHash, 'admin', 'gp-parwa']);
        db.run('INSERT INTO users (username, password, role, gpId) VALUES (?, ?, ?, ?)', ['staff-parwa', passwordHash, 'staff', 'gp-parwa']);
        
        // Borwha accounts
        db.run('INSERT INTO users (username, password, role, gpId) VALUES (?, ?, ?, ?)', ['admin-borwha', passwordHash, 'admin', 'gp-borwha']);
        db.run('INSERT INTO users (username, password, role, gpId) VALUES (?, ?, ?, ?)', ['staff-borwha', passwordHash, 'staff', 'gp-borwha']);
        
        // Lakhmapur accounts
        db.run('INSERT INTO users (username, password, role, gpId) VALUES (?, ?, ?, ?)', ['admin-lakhmapur', passwordHash, 'admin', 'gp-lakhmapur']);
        
        // Legacy 'admin' and 'staff' mapped to parwa for compatibility
        db.run('INSERT INTO users (username, password, role, gpId) VALUES (?, ?, ?, ?)', ['admin', passwordHash, 'admin', 'gp-parwa']);
        db.run('INSERT INTO users (username, password, role, gpId) VALUES (?, ?, ?, ?)', ['staff', passwordHash, 'staff', 'gp-parwa']);
    ...
    db.get('SELECT COUNT(*) as count FROM rtc_records', [], (err, row) => {
      ...
      if (row.count === 0) {
        ...
        const dummyRtc = [
          // Parwa land records
          { surveyNumber: "101/A", ownerName: "रामराव नामदेवराव ठोंबरे", village: "पारवा", gpId: "gp-parwa", fileUrl: "/uploads/rtc_dummy_101.pdf" },
          // Borwha land records
          { surveyNumber: "105/B", ownerName: "शरद सुखदेव देशमुख", village: "बोरव्हा", gpId: "gp-borwha", fileUrl: "/uploads/rtc_dummy_105.pdf" },
          // Lakhmapur land records
          { surveyNumber: "202", ownerName: "संजय पुंडलिक ठाकरे", village: "लखमापूर", gpId: "gp-lakhmapur", fileUrl: "/uploads/rtc_dummy_202.pdf" }
        ];
    ...
    // Seed gp-masolaBk tenant, users and RTC records
    const now = new Date().toLocaleString('mr-IN');
    db.run("INSERT OR IGNORE INTO grampanchayats (gpId, name) VALUES ('gp-masolaBk', 'मसोला (खुर्द)')", (err) => {
      if (err) return console.error('Error seeding Masola GP:', err.message);
      
      // Seed Masola users once GP is guaranteed to be present
      db.get("SELECT COUNT(*) as count FROM users WHERE gpId = 'gp-masolaBk'", [], async (err, row) => {
        if (err) return console.error(err);
        if (row.count === 0) {
          const passwordHash = await bcrypt.hash('gp2025!', 10);
          db.run("INSERT INTO users (username, password, role, gpId) VALUES ('admin-masola', ?, 'admin', 'gp-masolaBk')", [passwordHash]);
          db.run("INSERT INTO users (username, password, role, gpId) VALUES ('staff-masola', ?, 'staff', 'gp-masolaBk')", [passwordHash]);
          console.log('Seeded Masola multi-tenant user accounts.');
        }
      });

      // Seed Masola RTC/Land records
      db.get("SELECT COUNT(*) as count FROM rtc_records WHERE gpId = 'gp-masolaBk'", [], (err, row) => {
        if (err) return console.error(err);
        if (row.count === 0) {
          db.run("INSERT INTO rtc_records (surveyNumber, ownerName, village, gpId, fileUrl, createdAt) VALUES ('53/A', 'रामराव नामदेवराव ठोंबरे', 'मसोला बुद्रुक', 'gp-masolaBk', '/uploads/rtc_dummy_101.pdf', ?)", [now]);
          db.run("INSERT INTO rtc_records (surveyNumber, ownerName, village, gpId, fileUrl, createdAt) VALUES ('74/B', 'शरद सुखदेव देशमुख', 'मसोला बुद्रुक', 'gp-masolaBk', '/uploads/rtc_dummy_105.pdf', ?)", [now]);
          console.log('Seeded Masola multi-tenant RTC records.');
        }
      });
    });
  });
